...
export abstract class BaseQuery<TInput, TOutput> extends BaseComponent<TInput, TOutput> {
    ...
    // For now, we trust the runtime setup (BaseUseCase) to NOT provide ctx.commit for Queries.
    ...
    protected async _run(ctx: SystemCtx, input: TInput): Promise<TOutput> {
        ...
        if (ctx.commit) {
            throw new AgentGuidanceError(
                ...
                "Check that this UseCase extends BaseQuery and not BaseCommand. Ensure you are not passing a 'write' context to a read-only Query."
            );
